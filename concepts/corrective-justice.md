...
```mermaid
classDiagram
    class Excess["Excess (wrongdoer's gain)"]
    class Deficiency["Deficiency (victim's loss)"]
    class EqualPosition["Equal Position (the just result)"]
    Excess -- EqualPosition : equal distance above
    Deficiency -- EqualPosition : equal distance below
```
